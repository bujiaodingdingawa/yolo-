...
def addMouseCallback(winname, param, callbackFunc=None, info="%"):
    ...
    def default_callbackFunc(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            A = [param[y][x][0], param[y][x][1], param[y][x][2]]
            #(1.3-param[y][x][1]/300)-60*(param[y][x][1]/150)
            print(A[0]+230, A[1]+137, A[2])
            f = open("G:/chuanshu.txt", "w")
            dets = np.array(A)
            np.savetxt("G:/chuanshu.txt", dets, fmt='%f', delimiter='/n')
...
def get_video_capture(video_path, width=None, height=None, fps=None):
    ...
    :return:
    """
    video_cap = cv2.VideoCapture(video_path ,cv2.CAP_DSHOW)
    # 设置分辨率
    video_cap.set(cv2.CAP_PROP_FRAME_WIDTH, 2560)
    video_cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 960)
